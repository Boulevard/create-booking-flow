...
export const useConfig = () => {
    ...
    const mapType =
        process.env.NEXT_PUBLIC_MAP_TYPE === 'Google' ? MapType.Google : MapType.MapBox ?? MapType.MapBox
    const flowType =
        process.env.NEXT_PUBLIC_FLOW_TYPE === 'SelectServiceFirst' ?
            FlowType.SelectServiceFirst : FlowType.SelectLocationFirst ?? FlowType.SelectLocationFirst
    const dateTimeType =
        process.env.NEXT_PUBLIC_DATE_TIME_TYPE === 'ShowTimeForManyDays' ?
            DateTimeType.ShowTimeForManyDays : DateTimeType.ShowTimeForOneDay ?? DateTimeType.ShowTimeForOneDay
    const displayAppSettings =
        process.env.NEXT_PUBLIC_DISPLAY_APP_SETTINGS === 'Yes' ?
            DisplayAppSettingsPopup.Yes : DisplayAppSettingsPopup.No ?? DisplayAppSettingsPopup.No
    return {
